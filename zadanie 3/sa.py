...
def sym_wyz(ilosc_zadan,temperatura,wspol_schladzania,wybor):
    ...
    wyb=2
    # wybor=1 #wybor dzialania alg: 1 - insert, 2 - swap

    #wybor temp wplywa jasno na ilosc ogolnej iteracji
    # temperatura = 3e+400
    # temperatura = 3e+300
    # temperatura = 3e+1
    # temperatura = 3e-100
    ...
    while ilosc_interacji != ilosc_powtorzen: #dopoki ilosc wykonania petli != ilosc_powtorzen, to dzialaj
        ...
        cmax_akt = calc_cmax(list_zad_los_tmp) #obliczenie cmaxa po operacji insert/swap
        ...
        while (cmax_best == cmax_akt): #gdyby cmax po oepracji swpa/insert byl taki sam, to ponownie przeprowadz operacje
            ...
            # print('wysietl:', cmax_best, cmax_akt, akt_ilosc_prob)
            # insert
            if wybor == 1:
                list_zad_los_tmp = copy.deepcopy(list_zad_los)
            ...
            akt_ilosc_prob += 1
            if maks_ilosc_prob == akt_ilosc_prob:
                cmax_akt+=1


        #etap 3 liczenie prawdopodobienstwa i decyzja o ruchu
        # print(type(cmax_akt - cmax_best))
        ################################### Boltzman ###################################
        Boltzman =0
        war_do_prawd=0
        if wyb==2:
            ...
            # print(cmax_akt-cmax_best,temperatura,war_do_prawd)
            # if war_do_prawd >= 0:
            #     prawd=1 #bo, gdy war_do_prawd >=0 do exp(war_do_prawd) rowniez >=0
            #     Boltzman=1
            if cmax_akt - cmax_best <= 0:
                Boltzman = 1
            else:
                Boltzman = 1/(1+2*math.exp(war_do_prawd))
                #prawd = math.exp((cmax_akt - cmax_best) / temperatura)

            if Boltzman > losuj.uniform(0,0.33):
                    print(' ',cmax_best,'        ',cmax_akt,'  Iteracja nr {0}, wybor trybu {1}, wstawianie {2} '.format(ilosc_interacji,wyb,wybor))
                    cmax_best=cmax_akt
                    old_best_kolejka=copy.deepcopy(list_zad_los)
        ...
        # if bez_rezultatu == 30000:
        #     print('zmiana tablic i bez_rezultatu=0')
        #     list_zad_los = copy.deepcopy(old_best_kolejka)


        # print((cmax_akt - cmax_best) / temperatura)
        # print(math.exp((cmax_akt - cmax_best) / temperatura))
        ...
        #etap 5 ilosc_interacji = 60000 lub break ze wzgledu na niska temp.
